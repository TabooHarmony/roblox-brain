...
name: roblox-testing
description: >
  Testing patterns for Roblox: TestEZ, mocking Roblox services, dependency injection,
  CI/CD with Lune, and automated quality workflows. Use when writing tests or setting up
...
# Roblox Testing Patterns

## When to Use

Load this skill when the task involves:

- Writing unit tests for Luau modules
- Setting up TestEZ or other testing frameworks
- Mocking Roblox services for testability
- Configuring CI/CD pipelines (GitHub Actions, Lune)
- Refactoring code for testability (dependency injection)
- Debugging flaky tests or test infrastructure

**Hand off to other skills when:**

- General Luau syntax and patterns → `roblox-luau-core`
- Type system design → `roblox-luau-types`
- OOP and module architecture → `roblox-luau-patterns`
- Security validation patterns → `roblox-security`
...
[TestEZ](https://github.com/Roblox/testez) is the standard BDD-style testing framework for Roblox.

### Installation

Add to `wally.toml` as a dev-dependency:
...
The key to testable Roblox code. Inject services as parameters instead of calling `game:GetService()` directly.
...
Since engine services are unavailable in external environments (like CI), use mocks to simulate behavior.
...
Integration tests verify data flow between multiple modules.

### RemoteEvent Flows

Use mock remotes to test client-server communication logic without a network:
...
Test saving in one module and retrieving in another:
...
[Lune](https://github.com/lune-org/lune) is a standalone Luau runtime for running scripts outside Roblox Studio.
