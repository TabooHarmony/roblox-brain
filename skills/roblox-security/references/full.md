...
An archived model can reveal scripts, remotes, `DataStore` calls, or a receipt callback, but static presence does not establish authority, reachability, ownership, or correctness. Use the artifact to locate the trust boundary, then inspect the actual handler and validate the runtime transition.

- mixed `Script`, `LocalScript`, and `ModuleScript` contexts require an explicit authority map;
- a `ProcessReceipt` or `DataStore` call still needs the domain-specific idempotency, session, schema, and failure checks;
- dynamic code, numeric requires, and obfuscation-like names are review triggers, not proof of malware or exploitability;
- keep static findings separate from observed runtime behavior and label missing evidence.

## Risk routing in shipped games

<!-- temporal: 2026-08 -->

In shipped Roblox games, static risk signals appear in many places, yet the core local risk signals (dynamic code, obfuscation-like names, numeric requires) concentrate in a small minority of individual script units. The counts are not a safety certification, but they show why a reviewer should route individual units and subtrees instead of rejecting a whole place because one script or name matched. Shipped, successful games trigger these signals routinely, usually via anti-exploit tooling.

Use this sequence:

1. locate the exact unit, hierarchy path, and source scan that triggered review;
2. inspect neighboring callers, required modules, remotes, assets, and runtime placement;
3. classify the signal as executable behavior, a comment/name hit, provenance concern, or unresolved;
4. expand the review only when reachability or a trust-boundary connection is demonstrated;
5. keep the artifact's unrelated useful content available while the flagged subtree remains untrusted.

These routing signals are triage evidence, not a verdict about malware, theft, licensing, or runtime exploitability.
